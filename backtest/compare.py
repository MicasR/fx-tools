...
def main():
    ...
    ev = dict(horizons=(4, 8, 16), atr_mult=args.atr_mult, rr=args.rr)
...
def _plots(df, sym, args, vs_sig, vp_sig, vs, vp):
    ...
    # 1) Equity curves (cumulative R from the ATR bracket)
    fig, ax = plt.subplots(figsize=(12, 5))
    ax.plot(np.cumsum(vs["rmults"]), label=f"VolumeSpike (n={vs['n_signals']})", lw=1.8)
    ax.plot(np.cumsum(vp["rmults"]), label=f"VolumeVPattern (n={vp['n_signals']})", lw=1.8)
    ...
    metrics = [
        ...
        ("bracket win%", vs["bracket_winrate"] * 100, vp["bracket_winrate"] * 100),
        ("avg R", vs["bracket_avg_R"], vp["bracket_avg_R"]),
    ]
