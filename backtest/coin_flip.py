"""
coin_flip.py — geofloor s200 TP3 @1:2000 with the February monster called TAILS.
The op survived its 53.5-lot add by $0.33 (< the $0.40 spread); this shows the
strategy's record if that bar had clipped it: monster R -> -1.0.  Post-hoc
substitution (conservative: ignores the 1-2 extra ops the freed Feb 6-9 slot
might have taken).  Champion row included per standing rule.
"""
import numpy as np
...
if __name__ == "__main__":
    ...
    geo = run_tf("XAU", tf="M15", triggers=("bounce",), smaP=7, sizing="geofloor",
                 slowP=200, unit="base", mult=0.01, prog_step=1.7, lev=2000.0)
    print(HDR)
    row("champion geo1.7", champ[0], champ[3])
    row("geofloor s200 (heads)", geo[0], geo[3])
    row("geofloor s200 (TAILS)", geo[0], geo[3], flip_top=True)
